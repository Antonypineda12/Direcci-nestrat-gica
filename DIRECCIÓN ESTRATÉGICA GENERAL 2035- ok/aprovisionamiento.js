document.addEventListener('DOMContentLoaded', () => {
    ...
    const downloadPdfBtn = document.getElementById('downloadPdfBtn'); // Asumo que este botón ya existe en tu HTML de Aprovisionamiento

    const contentToCapture = document.body;

    let chartInstance = null;

    // Aumentar el número total de preguntas a 20 para el checklist de Aprovisionamiento
    ...
    function updateSummary() {
        ...
        // --- NUEVA LÍNEA: Guardar el porcentaje en localStorage para Aprovisionamiento ---
        localStorage.setItem('aprovisionamientoCompliancePercentage', percentage.toFixed(0)); // Guardar como entero

        updateChart(complianceCounts);
    }

    // Función para crear/actualizar el gráfico (sin cambios, solo se asegura de que existe)
    ...
    // --- Función para Descargar PDF (Copiada del código anterior para esta página) ---
    ...
    try {
        ...
        doc.save('checklist_aprovisionamiento.pdf');
    } catch (error) {
