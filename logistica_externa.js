document.addEventListener('DOMContentLoaded', () => {
    ...
    const downloadPdfBtn = document.getElementById('downloadPdfBtn'); // Asumo que este botón ya existe en tu HTML

    const contentToCapture = document.body; // Captura todo el body para el PDF

    let chartInstance = null;

    const totalQuestions = 20; // Hay 20 preguntas en el HTML de Logística Externa
    ...
    function updateSummary() {
        ...
        // --- Guardar el porcentaje en localStorage para Logística Externa ---
        localStorage.setItem('logisticaExternaCompliancePercentage', percentage.toFixed(0)); // Guardar como entero
    ...
    try {
        ...
        doc.save('checklist_logistica_externa.pdf');
    } catch (error) {
    ...
    // Carga inicial del resumen al cargar la página
    updateSummary();
});
